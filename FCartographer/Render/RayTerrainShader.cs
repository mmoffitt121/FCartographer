    ...
    public class RayTerrainShader : Renderer
    {
        ...
        private void RenderShadows()
        {
            ...
            for (int i = 0; i < wid * hei; i += 4)
            {
                ...
                float h = inp[i];
                float luminosity = 1f;
                while (x < wid / 4 && x >= 0 && y < hei && y >= 0 && h <= 255 && h >= 0)
                {
                ...
                outp[i + 2] = (byte)Lerper.Lerp(Math.Clamp(128, 0, 255), outp[i + 2], 1 - opacity);
                outp[i + 1] = (byte)Lerper.Lerp(Math.Clamp(128, 0, 255), outp[i + 1], 1 - opacity);
                outp[i + 0] = (byte)Lerper.Lerp(Math.Clamp(128, 0, 255), outp[i + 0], 1 - opacity);
            }
